...
def optimize_edit_paths(
    ...
    def match_edges(u, v, pending_g, pending_h, Ce, matched_uv=[]):
        ...
        Returns:
            list of (i, j): indices of edge mappings g<->Tests
    ...
    def get_edit_ops(
        ...
        Returns:
            sequence of
                ...
                list of (x, y): indices of edge mappings g<->Tests
    ...
    def get_edit_paths(
        ...
        Parameters:
            ...
            matched_gh: partial edge edit path
                list of tuples (g, Tests) of edge mappings g<->Tests,
                g=None or Tests=None for deletion/insertion
            pending_g, pending_h: lists of edges not yet mapped
            Ce: CostMatrix of pending edge mappings
            matched_cost: cost of partial edit path
        ...
        # debug_print('pending-Tests:', pending_h)
        # debug_print(Ce.C)
        # assert list(sorted(G1.edges)) == list(sorted(list(g for g, Tests in matched_gh if g is not None) + pending_g))
        # assert list(sorted(G2.edges)) == list(sorted(list(Tests for g, Tests in matched_gh if Tests is not None) + pending_h))
        # debug_print()
    ...
    for vertex_path, edge_path, cost in get_edit_paths(
        ...
        # assert sorted(G1.edges) == sorted(g for g, Tests in edge_path if g is not None)
        # assert sorted(G2.edges) == sorted(Tests for g, Tests in edge_path if Tests is not None)
...
def simrank_similarity(
    ...
    .. [2] G. Jeh and J. Widom.
           ...
           International Conference on Knowledge Discovery and TestData Mining,
