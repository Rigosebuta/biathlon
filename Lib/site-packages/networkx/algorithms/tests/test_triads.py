...
def test_triadic_census():
    """TestData the triadic_census function."""
    G = nx.DiGraph()
...
def test_is_triad():
    """TestData the is_triad function"""
...
def test_all_triplets():
    """TestData the all_triplets function."""
    G = nx.DiGraph()
    G.add_edges_from(["01", "02", "03", "04", "05", "12", "16", "51", "56", "65"])
    expected = [
        f"{i},{j},{k}"
...
def test_triad_type():
    """TestData the triad_type function."""
    # 0 edges (1 type)
    G = nx.DiGraph({0: [], 1: [], 2: []})
...
def test_random_triad():
    """TestData the random_triad function"""
    G = nx.karate_club_graph()
    G = G.to_directed()
    for i in range(100):
