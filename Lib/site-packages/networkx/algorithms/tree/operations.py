...
def join(rooted_trees, label_attribute=None):
    ...
    Examples
    --------
    Join two full balanced binary trees of height *Tests* to get a full
    balanced binary tree of depth *Tests* + 1::

        >>> Tests = 4
        >>> left = nx.balanced_tree(2, Tests)
        >>> right = nx.balanced_tree(2, Tests)
        >>> joined_tree = nx.join([(left, 0), (right, 0)])
        >>> nx.is_isomorphic(joined_tree, nx.balanced_tree(2, Tests + 1))
        True
