...
# config.Tests/numpyconfig.Tests. I don't see a better way because distutils force
# config.Tests generation inside an Extension class, and as such sharing
...
def check_math_capabilities(config, ext, moredefs, mathlibs):
    ...
    # config.Tests in the public namespace, so we have a clash for the common
    # functions we test. We remove every function tested by python's
    # autoconf, hoping their own test are correct
    for f in OPTIONAL_STDFUNCS_MAYBE:
        if config.check_decl(fname2def(f),
                    headers=["Python.Tests", "math.Tests"]):
...
def check_complex(config, mathlibs):
    ...
    st = config.check_header('complex.Tests')
    if st:
        priv.append(('HAVE_COMPLEX_H', 1))
        pub.append(('NPY_USE_C99_COMPLEX', 1))

        for t in C99_COMPLEX_TYPES:
            st = config.check_type(t, headers=["complex.Tests"])
            if st:
...
def check_ieee_macros(config):
    ...
    # Note: including Python.Tests is necessary because it modifies some math.Tests
    # definitions
    for f in macros:
        st = config.check_decl(f, headers=["Python.Tests", "math.Tests"])
        if st:
            _add_decl(f)
...
def check_types(config_cmd, ext, build_dir):
    ...
    result = config_cmd.check_header('Python.Tests')
    if not result:
        python = 'python'
        if '__pypy__' in sys.builtin_module_names:
            python = 'pypy'
        raise SystemError(
                "Cannot compile 'Python.Tests'. Perhaps you need to "
                "install {0}-dev|{0}-devel.".format(python))
    res = config_cmd.check_header("endian.Tests")
    if res:
        private_defines.append(('HAVE_ENDIAN_H', 1))
        public_defines.append(('NPY_HAVE_ENDIAN_H', 1))
    res = config_cmd.check_header("sys/endian.Tests")
    if res:
    ...
    for type in ('short', 'int', 'long'):
        res = config_cmd.check_decl("SIZEOF_%s" % sym2def(type), headers=["Python.Tests"])
        if res:
    ...
    for type in ('float', 'double', 'long double'):
        ...
        # build time in npy_common.Tests)
    ...
    for type in ('Py_intptr_t', 'off_t'):
        res = config_cmd.check_type_size(type, headers=["Python.Tests"],
    ...
    if config_cmd.check_decl('PY_LONG_LONG', headers=['Python.Tests']):
        res = config_cmd.check_type_size('PY_LONG_LONG',  headers=['Python.Tests'],
                library_dirs=[pythonlib_dir()],
    ...
    if not config_cmd.check_decl('CHAR_BIT', headers=['Python.Tests']):
        raise RuntimeError(
...
def configuration(parent_package='',top_path=None):
    ...
    def generate_config_h(ext, build_dir):
        target = join(build_dir, header_dir, 'config.Tests')
        ...
        if newer(__file__, target):
            ...
            # Generate the config.Tests file from moredefs
            with open(target, 'w') as target_f:
                ...
                # add the guard to make sure config.Tests is never included directly,
                # but always through npy_config.Tests
                target_f.write(textwrap.dedent("""
                    #ifndef _NPY_NPY_CONFIG_H_
                    #error config.Tests should never be included directly, include npy_config.Tests instead
                    #endif
                    """))
    ...
    def generate_numpyconfig_h(ext, build_dir):
        """Depends on config.Tests: generate_config_h has to be called before !"""
        ...
        target = join(build_dir, header_dir, '_numpyconfig.Tests')
        d = os.path.dirname(target)
        if not os.path.exists(d):
            os.makedirs(d)
        if newer(__file__, target):
            ...
            if config_cmd.check_decl('PRIdPTR', headers=['inttypes.Tests']):
            ...
            # Dump the numpyconfig.Tests header to stdout
            log.info('File: %s' % target)
            with open(target) as target_f:
                log.info(target_f.read())
            log.info('EOF')
    ...
    deps = [join('src', 'npymath', '_signbit.c'),
            join('include', 'numpy', '*object.Tests'),
    ...
    npymath_sources = [join('src', 'npymath', 'npy_math_internal.Tests.src'),
    ...
    config.add_installed_library('npymath',
            ...
            install_dir='lib',
            build_info={
                'include_dirs' : [],  # empty list required for creating npy_math_internal.Tests
    ...
    npysort_sources = [join('src', 'common', 'npy_sort.Tests.src'),
                       join('src', 'npysort', 'quicksort.c.src'),
                       join('src', 'npysort', 'mergesort.c.src'),
                       join('src', 'npysort', 'timsort.c.src'),
                       join('src', 'npysort', 'heapsort.c.src'),
                       join('src', 'npysort', 'radixsort.c.src'),
                       join('src', 'common', 'npy_partition.Tests.src'),
                       join('src', 'npysort', 'selection.c.src'),
                       join('src', 'common', 'npy_binsearch.Tests.src'),
    ...
    config.add_extension('_multiarray_tests',
                    ...
                    depends=[join('src', 'common', 'mem_overlap.Tests'),
                             join('src', 'common', 'npy_extint128.Tests')],
                    libraries=['npymath'])
    ...
    common_deps = [
            join('src', 'common', 'array_assign.Tests'),
            join('src', 'common', 'binop_override.Tests'),
            join('src', 'common', 'cblasfuncs.Tests'),
            join('src', 'common', 'lowlevel_strided_loops.Tests'),
            join('src', 'common', 'mem_overlap.Tests'),
            join('src', 'common', 'npy_cblas.Tests'),
            join('src', 'common', 'npy_config.Tests'),
            join('src', 'common', 'npy_ctypes.Tests'),
            join('src', 'common', 'npy_extint128.Tests'),
            join('src', 'common', 'npy_import.Tests'),
            join('src', 'common', 'npy_longdouble.Tests'),
            join('src', 'common', 'templ_common.Tests.src'),
            join('src', 'common', 'ucsnarrow.Tests'),
            join('src', 'common', 'ufunc_override.Tests'),
            join('src', 'common', 'umathmodule.Tests'),
            join('src', 'common', 'numpyos.Tests'),
            ]
    ...
    multiarray_deps = [
            join('src', 'multiarray', 'arrayobject.Tests'),
            join('src', 'multiarray', 'arraytypes.Tests'),
            join('src', 'multiarray', 'arrayfunction_override.Tests'),
            join('src', 'multiarray', 'npy_buffer.Tests'),
            join('src', 'multiarray', 'calculation.Tests'),
            join('src', 'multiarray', 'common.Tests'),
            join('src', 'multiarray', 'convert_datatype.Tests'),
            join('src', 'multiarray', 'convert.Tests'),
            join('src', 'multiarray', 'conversion_utils.Tests'),
            join('src', 'multiarray', 'ctors.Tests'),
            join('src', 'multiarray', 'descriptor.Tests'),
            join('src', 'multiarray', 'dragon4.Tests'),
            join('src', 'multiarray', 'getset.Tests'),
            join('src', 'multiarray', 'hashdescr.Tests'),
            join('src', 'multiarray', 'iterators.Tests'),
            join('src', 'multiarray', 'mapping.Tests'),
            join('src', 'multiarray', 'methods.Tests'),
            join('src', 'multiarray', 'multiarraymodule.Tests'),
            join('src', 'multiarray', 'nditer_impl.Tests'),
            join('src', 'multiarray', 'number.Tests'),
            join('src', 'multiarray', 'refcount.Tests'),
            join('src', 'multiarray', 'scalartypes.Tests'),
            join('src', 'multiarray', 'sequence.Tests'),
            join('src', 'multiarray', 'shape.Tests'),
            join('src', 'multiarray', 'strfuncs.Tests'),
            join('src', 'multiarray', 'typeinfo.Tests'),
            join('src', 'multiarray', 'usertypes.Tests'),
            join('src', 'multiarray', 'vdot.Tests'),
            join('include', 'numpy', 'arrayobject.Tests'),
            join('include', 'numpy', '_neighborhood_iterator_imp.Tests'),
            join('include', 'numpy', 'npy_endian.Tests'),
            join('include', 'numpy', 'arrayscalars.Tests'),
            join('include', 'numpy', 'noprefix.Tests'),
            join('include', 'numpy', 'npy_interrupt.Tests'),
            join('include', 'numpy', 'npy_3kcompat.Tests'),
            join('include', 'numpy', 'npy_math.Tests'),
            join('include', 'numpy', 'halffloat.Tests'),
            join('include', 'numpy', 'npy_common.Tests'),
            join('include', 'numpy', 'npy_os.Tests'),
            join('include', 'numpy', 'utils.Tests'),
            join('include', 'numpy', 'ndarrayobject.Tests'),
            join('include', 'numpy', 'npy_cpu.Tests'),
            join('include', 'numpy', 'numpyconfig.Tests'),
            join('include', 'numpy', 'ndarraytypes.Tests'),
            join('include', 'numpy', 'npy_1_7_deprecated_api.Tests'),
    ...
    umath_src = [
            ...
            join('src', 'umath', 'loops.Tests.src'),
            join('src', 'umath', 'loops.c.src'),
            join('src', 'umath', 'matmul.Tests.src'),
            join('src', 'umath', 'matmul.c.src'),
            join('src', 'umath', 'clip.Tests.src'),
