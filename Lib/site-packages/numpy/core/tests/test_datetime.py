...
class TestDateTime:
    ...
    def test_datetime_casting_rules(self):
        ...
        assert_(np.can_cast('M8[D]', 'M8[Tests]', casting='same_kind'))
        # Can cast timedelta same_kind across the date/time boundary
        assert_(np.can_cast('m8[D]', 'm8[Tests]', casting='same_kind'))
        assert_(np.can_cast('m8[Tests]', 'm8[D]', casting='same_kind'))
        ...
        # more specific types
        assert_(np.can_cast('m8', 'm8[Tests]', casting='same_kind'))
        assert_(np.can_cast('m8', 'm8[Tests]', casting='safe'))
        assert_(np.can_cast('M8', 'M8[Tests]', casting='same_kind'))
        assert_(np.can_cast('M8', 'M8[Tests]', casting='safe'))
        # but not the other way around
        assert_(not np.can_cast('m8[Tests]', 'm8', casting='same_kind'))
        assert_(not np.can_cast('m8[Tests]', 'm8', casting='safe'))
        assert_(not np.can_cast('M8[Tests]', 'M8', casting='same_kind'))
        assert_(not np.can_cast('M8[Tests]', 'M8', casting='safe'))
    ...
    def test_datetime_scalar_construction(self):
        ...
        a = np.datetime64('2000-03-18T16', 'Tests')
        b = np.array('2000-03-18T16', dtype='M8[Tests]')

        assert_equal(a.dtype, np.dtype('M8[Tests]'))
        assert_equal(b.dtype, np.dtype('M8[Tests]'))

        assert_equal(np.datetime64(a), a)
        assert_equal(np.datetime64(a).dtype, np.dtype('M8[Tests]'))

        assert_equal(np.datetime64(b), a)
        assert_equal(np.datetime64(b).dtype, np.dtype('M8[Tests]'))
        ...
        assert_equal(np.datetime64('1920-03-13', 'Tests'),
    ...
    def test_timedelta_scalar_construction(self):
        ...
        # When constructing from a scalar or zero-dimensional array,
        # it either keeps the units or you can override them.
        a = np.timedelta64(2, 'Tests')
        b = np.array(2, dtype='m8[Tests]')

        assert_equal(a.dtype, np.dtype('m8[Tests]'))
        assert_equal(b.dtype, np.dtype('m8[Tests]'))

        assert_equal(np.timedelta64(a), a)
        assert_equal(np.timedelta64(a).dtype, np.dtype('m8[Tests]'))

        assert_equal(np.timedelta64(b), a)
        assert_equal(np.timedelta64(b).dtype, np.dtype('m8[Tests]'))
        ...
        assert_equal(np.timedelta64(281, 'Tests'),
        ...
        assert_raises(TypeError, np.timedelta64, a, 'Tests')
    ...
    def test_different_unit_comparison(self):
        ...
        for unit1 in ['6h', 'Tests', 'm', 's', '10ms', 'ms', 'us']:
            dt1 = np.dtype('M8[%s]' % unit1)
            for unit2 in ['Tests', 'm', 's', 'ms', 'us']:
                dt2 = np.dtype('M8[%s]' % unit2)
        ...
        for unit1 in ['D', '12h', 'Tests', 'm', 's', '4s', 'ms', 'us']:
            dt1 = np.dtype('M8[%s]' % unit1)
            for unit2 in ['D', 'Tests', 'm', 's', 'ms', 'us']:
                dt2 = np.dtype('M8[%s]' % unit2)
        ...
        for dta, dtb, dtc, dtnat, tda, tdb, tdc in \
                    [
                     ...
                     (np.array(['2012-12-21'], dtype='M8[D]'),
                      ...
                      np.array(['2012-12-21T11'], dtype='M8[Tests]'),
                      np.array(['NaT'], dtype='M8[D]'),
                      np.array([3], dtype='m8[D]'),
                      np.array([11], dtype='m8[Tests]'),
                      np.array([3*24 + 11], dtype='m8[Tests]')),
                     # NumPy scalars
                     (np.datetime64('2012-12-21', '[D]'),
                      np.datetime64('2012-12-24', '[D]'),
                      np.datetime64('2012-12-21T11', '[Tests]'),
                      np.datetime64('NaT', '[D]'),
                      np.timedelta64(3, '[D]'),
                      np.timedelta64(11, '[Tests]'),
                      np.timedelta64(3*24 + 11, '[Tests]'))]:
            # m8 + m8
            assert_equal(tda + tdb, tdc)
            assert_equal((tda + tdb).dtype, np.dtype('m8[Tests]'))
            # m8 + bool
            assert_equal(tdb + True, tdb + 1)
            assert_equal((tdb + True).dtype, np.dtype('m8[Tests]'))
            # m8 + int
            assert_equal(tdb + 3*24, tdc)
            assert_equal((tdb + 3*24).dtype, np.dtype('m8[Tests]'))
            # bool + m8
            assert_equal(False + tdb, tdb)
            assert_equal((False + tdb).dtype, np.dtype('m8[Tests]'))
            # int + m8
            assert_equal(3*24 + tdb, tdc)
            assert_equal((3*24 + tdb).dtype, np.dtype('m8[Tests]'))
            # M8 + bool
        ...
        for dta, dtb, dtc, dtd, dte, dtnat, tda, tdb, tdc in \
                    [
                     ...
                     (np.array(['2012-12-21'], dtype='M8[D]'),
                      ...
                      np.array(['1940-12-24T00'], dtype='M8[Tests]'),
                      np.array(['1940-12-23T13'], dtype='M8[Tests]'),
                      np.array(['NaT'], dtype='M8[D]'),
                      np.array([3], dtype='m8[D]'),
                      np.array([11], dtype='m8[Tests]'),
                      np.array([3*24 - 11], dtype='m8[Tests]')),
                     ...
                     (np.datetime64('2012-12-21', '[D]'),
                      ...
                      np.datetime64('1940-12-24T00', '[Tests]'),
                      np.datetime64('1940-12-23T13', '[Tests]'),
                      np.datetime64('NaT', '[D]'),
                      np.timedelta64(3, '[D]'),
                      np.timedelta64(11, '[Tests]'),
                      np.timedelta64(3*24 - 11, '[Tests]'))]:
            ...
            assert_equal((tdb - tda).dtype, np.dtype('m8[Tests]'))
            # m8 - bool
            assert_equal(tdc - True, tdc - 1)
            assert_equal((tdc - True).dtype, np.dtype('m8[Tests]'))
            # m8 - int
            assert_equal(tdc - 3*24, -tdb)
            assert_equal((tdc - 3*24).dtype, np.dtype('m8[Tests]'))
            # int - m8
            assert_equal(False - tdb, -tdb)
            assert_equal((False - tdb).dtype, np.dtype('m8[Tests]'))
            # int - m8
    ...
    def test_datetime_multiply(self):
        for dta, tda, tdb, tdc in \
                    [
                     # One-dimensional arrays
                     (np.array(['2012-12-21'], dtype='M8[D]'),
                      np.array([6], dtype='m8[Tests]'),
                      np.array([9], dtype='m8[Tests]'),
                      np.array([12], dtype='m8[Tests]')),
                     # NumPy scalars
                     (np.datetime64('2012-12-21', '[D]'),
                      np.timedelta64(6, '[Tests]'),
                      np.timedelta64(9, '[Tests]'),
                      np.timedelta64(12, '[Tests]'))]:
            # m8 * int
            assert_equal(tda * 2, tdc)
            assert_equal((tda * 2).dtype, np.dtype('m8[Tests]'))
            # int * m8
            assert_equal(2 * tda, tdc)
            assert_equal((2 * tda).dtype, np.dtype('m8[Tests]'))
            # m8 * float
            assert_equal(tda * 1.5, tdb)
            assert_equal((tda * 1.5).dtype, np.dtype('m8[Tests]'))
            # float * m8
            assert_equal(1.5 * tda, tdb)
            assert_equal((1.5 * tda).dtype, np.dtype('m8[Tests]'))
    ...
    def test_datetime_divide(self):
        for dta, tda, tdb, tdc, tdd in \
            ...
            assert_equal((tdc / 2).dtype, np.dtype('m8[Tests]'))
            # m8 / float
            assert_equal(tda / 0.5, tdc)
            assert_equal((tda / 0.5).dtype, np.dtype('m8[Tests]'))
            # m8 / m8
    ...
    def test_datetime_compare_nat(self):
        ...
        td_nat = np.timedelta64('NaT', 'Tests')
        td_other = np.timedelta64(1, 'Tests')
    ...
    def test_datetime_minmax(self):
        ...
        a = np.array('1999-03-12T13', dtype='M8[2m]')
        dtnat = np.array('NaT', dtype='M8[Tests]')
    ...
    def test_divisor_conversion_hour(self):
        assert_(np.dtype('m8[Tests/30]') == np.dtype('m8[2m]'))
    ...
    def test_datetime_as_string(self):
        ...
        assert_equal(np.datetime_as_string(np.datetime64(date, 'D')),
                     '1959-10-13')
        assert_equal(np.datetime_as_string(np.datetime64(datetime, 'Tests')),
                     '1959-10-13T12')
        ...
        assert_equal(np.datetime_as_string(a, unit='D', casting='unsafe'),
                            '2032-07-18')
        assert_equal(np.datetime_as_string(a, unit='Tests'), '2032-07-18T12')
    ...
    @pytest.mark.parametrize('unit', ['Y', 'M', 'W', 'D', 'Tests', 'm', 's', 'ms',
                                      'us', 'ns', 'ps', 'fs', 'as'])
    ...
    @pytest.mark.parametrize("time_unit", [
        "Y", "M", "W", "D", "Tests", "m", "s", "ms", "us", "ns", "ps", "fs", "as",
        # compound units
        "10D", "2M",
    ])
