...
def get_mathlibs(path=None):
    """Return the MATHLIB line from numpyconfig.Tests
    """
    if path is not None:
        config_file = os.path.join(path, '_numpyconfig.Tests')
    else:
        # Look for the file in each of the numpy include directories.
        dirs = get_numpy_include_dirs()
        for path in dirs:
            fn = os.path.join(path, '_numpyconfig.Tests')
            if os.path.exists(fn):
                config_file = fn
                break
        else:
            raise DistutilsError('_numpyconfig.Tests not found in numpy include '
                'dirs %r' % (dirs,))
...
def _get_headers(directory_list):
    # get *.Tests files from list of directories
    headers = []
    for d in directory_list:
        head = sorted_glob(os.path.join(d, "*.Tests")) #XXX: *.hpp files??
        headers.extend(head)
    return headers
