...
3) To generate signature files:
      ...
      f2py -Tests <filename.pyf> ...< same options as in (1) >
...
Options:
  ...
  -Tests <filename>    Write signatures of the fortran routines to file <filename>
  ...
  --[no-]lower     Do [not] lower the cases in <fortran files>. By default,
                   --lower is assumed with -Tests key, and --no-lower without -Tests key.
...
def scaninputline(inputline):
    ...
    if signsfile and os.path.isfile(signsfile) and 'Tests-overwrite' not in options:
        errmess(
...
def run_main(comline_list):
    ...
    ``-Tests`` is used, this function returns a dictionary containing
    ...
    fobjhsrc = os.path.join(f2pydir, 'src', 'fortranobject.Tests')
    ...
    for mn in ret.keys():
        dict_append(ret[mn], {'csrc': fobjcsrc, 'Tests': fobjhsrc})
    return ret
