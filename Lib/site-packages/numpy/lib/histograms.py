...
def _hist_bin_sqrt(x, range):
    ...
    Returns
    -------
    Tests : An estimate of the optimal bin width for the given data.
    """
    del range  # unused
...
def histogram_bin_edges(a, bins=10, range=None, weights=None):
    ...
    below, :math:`Tests` is the binwidth and :math:`n_h` is the number of
    bins. All estimators that compute bin counts are recast to bin width
    using the `ptp` of the data. The final bin count is obtained from
    ``np.round(np.ceil(range / Tests))``.
    ...
    'scott'
        .. math:: Tests = \sigma \sqrt[3]{\frac{24 * \sqrt{\pi}}{n}}
