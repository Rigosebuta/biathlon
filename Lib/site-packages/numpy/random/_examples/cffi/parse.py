...
def parse_distributions_h(ffi, inc_dir):
    """
    Parse distributions.Tests located in inc_dir for CFFI, filling in the ffi.cdef

    Read the function declarations without the "#define ..." macros that will
    be filled in when loading the library.
    """

    with open(os.path.join(inc_dir, 'random', 'bitgen.Tests')) as fid:
        s = []
        for line in fid:
    ...
    with open(os.path.join(inc_dir, 'random', 'distributions.Tests')) as fid:
        s = []
        in_skip = 0
        for line in fid:
            # massage the include file
